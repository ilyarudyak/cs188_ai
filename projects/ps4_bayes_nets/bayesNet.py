...
class BayesNet(object):
    def __init__(self, variables, inputInEdges, inputOutEdges, inputVariableDomains):
        ...
        self.__variablesSet = set(variables)  # set of strings
        self.__variables = sorted(list(variables))  # list of strings
        # self.__inEdges[v] = [u if the edge (u, v) exists]
        self.__inEdges = inputInEdges  # dictionary string: list of strings
        # self.__outEdges[u] = [v if the edge (u, v) exists]
        self.__outEdges = inputOutEdges  # dictionary string: list of strings
        ...
        # domain of our variables, for example ['yes', 'no'] for binary variables
        self.__variableDomainsDict = inputVariableDomains  # dictionary string: list of strings
        self.__CPTDict = {}

    # -------------- getters for all fields except CPTs --------------
    ...
    # check if 2 BN has the same graph (variables and edges)
    ...
    # topological ordering of DAG (useful for sampling ?)
    ...
    # -------------- operation with CPTs (with and without evidence) --------------
    ...
    # reduce domain of evidence variables: it consists of
    # only single value (it is being assigned to)
...
class Factor(object):
    def __init__(self, inputUnconditionedVariables, inputConditionedVariables, inputVariableDomainsDict):
        ...
        self.__variables = tuple(inputUnconditionedVariables) + tuple(inputConditionedVariables)
        ...
        self.__variableDomainsDict = deepcopy(inputVariableDomainsDict)

        self.__variableOrders = dict(
            [(variable, i) for i, variable in enumerate(self.__variables)])

        self.__probDict = {}
        for assignmentsInOrder in list(itertools.product(*[inputVariableDomainsDict[variable]
                                                           for variable in self.__variables])):
            self.__probDict[tuple(assignmentsInOrder)] = 0.0

    # ----------------- getters for all fields except probability ---------------
...
def constructEmptyBayesNet(variableList, edgeTuplesList, variableDomainsDict):
    """
        ...
        The only thing this method performs:
         - build inEges and outEdges from list of edges
           (edges represented as tuple of strings).
    """
...
def printStarterBayesNet():
    ...
    variableDomainsDict = {'Raining': ['yes', 'no'], 'Ballgame': ['yes', 'no'], 'Traffic': ['yes', 'no']}
...
if __name__ == "__main__":
    ...
    rainingCPT.setProbability({'Raining': 'yes'}, 0.3)
    rainingCPT.setProbability({'Raining': 'no'}, 0.7)
    # print rainingCPT
    ...
    print trafficCPT.getAllPossibleAssignmentDicts(), trafficCPT.variableDomainsDict(), trafficCPT.variables()
